...
class DNN_Ada(nn.Module):

    def __init__(self, inputs_dim, hidden_units, tags = None, dims = None, activation='relu', l2_reg=0, dropout_rate=0, use_bn=False,
                 init_std=0.0001, dice_dim=3, seed=1024, device='cpu'):
        super(DNN_Ada, self).__init__()
        self.dropout_rate = dropout_rate
        self.dropout = nn.Dropout(dropout_rate)
        self.seed = seed
        self.l2_reg = l2_reg
        ...
        for tag in tags:
            if not tag:
                self.linears.append(
                    nn.Linear(hidden_units[count], hidden_units[count + 1])
                    )
            else:
                self.linears.append(
                    nn.Linear(hidden_units[count], dims[count])
                    )
            count += 1

        self.after_linears = nn.ModuleList()
        count = 0
        for tag in tags:
            if not tag:
                self.after_linears.append(None)
            else:
                self.after_linears.append(
                    nn.Linear(dims[count], hidden_units[count + 1])
                    )
            count += 1
...
class DNN_Ada_Act(nn.Module):

    def __init__(self, inputs_dim, hidden_units, tags = None, dims = None, activation='relu', l2_reg=0, dropout_rate=0, use_bn=False,
                 init_std=0.0001, dice_dim=3, seed=1024, device='cpu'):
        super(DNN_Ada_Act, self).__init__()
        self.dropout_rate = dropout_rate
        self.dropout = nn.Dropout(dropout_rate)
        self.seed = seed
        self.l2_reg = l2_reg
