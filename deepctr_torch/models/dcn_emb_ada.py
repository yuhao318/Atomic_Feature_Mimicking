...
from ..layers import CrossNet, DNN, DNN_Ada


class DCN_EMB_Ada(BaseModel):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, num_sparse_features, emb_input_dims, emb_output_dims, 
                 tags = None, dims = None, cross_num=2, cross_parameterization='vector',
        ...
        super(DCN_EMB_Ada, self).__init__(linear_feature_columns=linear_feature_columns,
        ...
        if len(self.dnn_hidden_units) > 0 and self.cross_num > 0:
            dnn_linear_in_feature = self.compute_origin_input_dim(dnn_feature_columns, emb_output_dims) + dnn_hidden_units[-1]
        elif len(self.dnn_hidden_units) > 0:
            dnn_linear_in_feature = dnn_hidden_units[-1]
        elif self.cross_num > 0:
            dnn_linear_in_feature = self.compute_origin_input_dim(dnn_feature_columns, emb_output_dims)

        self.dnn_linear = nn.Linear(dnn_linear_in_feature, 1, bias=False).to(
            device)
        self.crossnet = CrossNet(in_features=self.compute_origin_input_dim(dnn_feature_columns, emb_output_dims),
    ...
    def forward(self, X):
        ...
        if len(sparse_embedding_list) > 0 and len(dense_value_list) > 0:
            ...
            sparse_dnn_input = torch.cat(sparse_embedding_list, dim=1)
            sparse_dnn_input_orig = torch.einsum("ijk,jkl->ijl", (sparse_dnn_input, self.emb_weight)) + self.emb_bias
            sparse_dnn_input_orig = torch.flatten(sparse_dnn_input_orig, start_dim = 1)

            sparse_dnn_input = torch.flatten(sparse_dnn_input, start_dim = 1)

            dense_dnn_input = torch.flatten(
                torch.cat(dense_value_list, dim=1), start_dim=1)

            dnn_input_orig = torch.cat([sparse_dnn_input_orig, dense_dnn_input], dim=-1)
            ...
            sparse_dnn_input = torch.cat(sparse_embedding_list, dim=1)
            sparse_dnn_input_orig = torch.einsum("ijk,jkl->ijl", (sparse_dnn_input, self.emb_weight)) + self.emb_bias
            dnn_input_orig = torch.flatten(sparse_dnn_input_orig, start_dim = 1)

            dnn_input = torch.flatten(sparse_dnn_input, start_dim = 1)
        ...
        if len(self.dnn_hidden_units) > 0 and self.cross_num > 0:  # Deep & Cross
            ...
            cross_out = self.crossnet(dnn_input_orig)
