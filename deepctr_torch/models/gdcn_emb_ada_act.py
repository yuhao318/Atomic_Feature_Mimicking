...
from ..layers import DNN, DNN_Ada_Act

class CrossNet(nn.Module):
    #  The core structure： gated corss layer.
    def __init__(self, input_dim, cn_layers=3,device='cpu'):
        super().__init__()
...
class GDCN_EMB_Ada_Act(BaseModel):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, num_sparse_features, emb_input_dims, emb_output_dims,  
        ...
        super(GDCN_EMB_Ada_Act, self).__init__(linear_feature_columns=linear_feature_columns,
        ...
        self.dnn_linear = nn.Linear(dnn_linear_in_feature, 1, bias=False).to(
            device)
        self.crossnet = CrossNet(input_dim=self.compute_origin_input_dim(dnn_feature_columns, emb_output_dims),
                                 cn_layers=cross_num, device=device)
