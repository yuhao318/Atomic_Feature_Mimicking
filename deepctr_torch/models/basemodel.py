...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        for epoch in range(initial_epoch, epochs):
            # print(do_validation, val_x.shape, val_y.shape)
            ...
            self.save_checkpoint(model, str(epoch) + ".pt")
    ...
    def predict(self, x, batch_size=256):
        ...
        sum_time = 0
        with torch.no_grad():
            for ind, x_test in tqdm(enumerate(test_loader)):
    ...
    def compute_origin_input_dim(self, feature_columns, sparse_feature_emb, include_sparse=True, include_dense=True, feature_group=False):
        sparse_feature_columns = list(
            filter(lambda x: isinstance(x, (SparseFeat, VarLenSparseFeat)), feature_columns)) if len(
            feature_columns) else []
        dense_feature_columns = list(
        ...
        if feature_group:
            ...
            sparse_input_dim = sum(sparse_feature_emb for feat in sparse_feature_columns)
        input_dim = 0
        if include_sparse:
            input_dim += sparse_input_dim
        if include_dense:
    ...
    def _get_optim(self, optimizer):
        if isinstance(optimizer, str):
            if optimizer == "sgd":
                ...
                optim = torch.optim.AdamW(self.parameters())  # 0.001
