...
from ..layers import SENETLayer, BilinearInteraction, DNN, DNN_Ada


class FiBiNET_EMB_Ada(BaseModel):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, num_sparse_features,  emb_input_dims, emb_output_dims, tags = None, dims = None,bilinear_type='interaction',
                 reduction_ratio=3, dnn_hidden_units=(128, 128), l2_reg_linear=1e-5,
                 l2_reg_embedding=1e-5, l2_reg_dnn=0, init_std=0.0001, seed=1024, dnn_dropout=0, dnn_activation='relu',
                 task='binary', device='cpu', gpus=None):
        super(FiBiNET_EMB_Ada, self).__init__(linear_feature_columns, dnn_feature_columns, l2_reg_linear=l2_reg_linear,
                                      l2_reg_embedding=l2_reg_embedding, init_std=init_std, seed=seed, task=task,
                                      device=device, gpus=gpus)
        self.linear_feature_columns = linear_feature_columns
        self.dnn_feature_columns = dnn_feature_columns
        self.filed_size = len(self.embedding_dict)
        self.final_embedding_size = emb_output_dims
        self.SE = SENETLayer(self.filed_size, reduction_ratio, seed, device)

        # self.emb_weight = nn.parameter.Parameter(torch.zeros([num_sparse_features, emb_input_dims, emb_output_dims]))
        # self.emb_bias = nn.parameter.Parameter(torch.zeros([1 ,num_sparse_features,  emb_output_dims]))

        self.register_parameter('emb_weight' , nn.parameter.Parameter(torch.zeros([num_sparse_features, emb_input_dims, emb_output_dims])))
        self.register_parameter('emb_bias' , nn.parameter.Parameter(torch.zeros([1 ,num_sparse_features,  emb_output_dims])))

        self.SE_Bilinear = BilinearInteraction(self.filed_size, self.final_embedding_size, bilinear_type, seed, device)
        self.Bilinear = BilinearInteraction(self.filed_size, self.final_embedding_size, bilinear_type, seed, device)
        self.dnn = DNN_Ada(self.compute_input_dim(dnn_feature_columns), dnn_hidden_units, tags = tags, dims = dims,
    ...
    def compute_input_dim(self, feature_columns, include_sparse=True, include_dense=True):
        ...
        embedding_size = self.final_embedding_size
    ...
    def forward(self, X):
        sparse_embedding_list, dense_value_list = self.input_from_feature_columns(X, self.dnn_feature_columns,
                                                                                  self.embedding_dict)
        sparse_embedding_input_o = torch.cat(sparse_embedding_list, dim=1)

        # print(sparse_embedding_input_o.shape)
        sparse_embedding_input = torch.einsum("ijk,jkl->ijl", (sparse_embedding_input_o, self.emb_weight)) + self.emb_bias
        ...
        # sparse_embedding_list, dense_value_list, senet_output, senet_bilinear_out, bilinear_out, temp1, dnn_input, final_logit
